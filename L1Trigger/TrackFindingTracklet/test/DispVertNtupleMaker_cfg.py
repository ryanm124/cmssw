...
import os
process = cms.Process("DisplacedVertex")
...
if GEOMETRY == "D49":
  ...
  inputMC = ["/store/mc/Phase2HLTTDRWinter20DIGI/HiddenGluGluH_mH-125_Phi-15_ctau-10_14TeV-pythia8_TuneCP5/GEN-SIM-DIGI-RAW/PU200_110X_mcRun4_realistic_v3-v2/240000/00D87D4F-81AB-4145-8E1F-EDFCD623EE4C.root"]
...
process.TFileService = cms.Service("TFileService", fileName = cms.string('DispTrackJetPDGID_PU200_'+GEOMETRY+'.root'), closeFileFast = cms.untracked.bool(True))
...
process.load('L1Trigger.TrackFindingTracklet.DisplacedVertexProducer_cff')
process.DispVert = cms.Path(process.DisplacedVertexProducer)
process.schedule = cms.Schedule(process.TTClusterStub,process.TTClusterStubTruth,process.dtc,process.TTTracksEmulationWithTruth,process.DispVert,process.ana)
